...
PointCloudXYZI::Ptr point_cloud_read(new pcl::PointCloud<pcl::PointXYZI>);
...
void SigHandle(int sig)
{
 ...
 int read(PointCloudXYZI::Ptr &point_cloud_read, char filename)
{
	if(pcl::io::loadPCDFile<pcl::PointXYZI>(filename, *point_cloud_read) == -1)
	{
		PCL_ERROR("Couldn't read file.\n")
		return (-1);
	}
	std::cout<<"Loaded file "<<point_cloud_read->width*point_cloud_read->height<<" data points"<<endl;
	return 0;
} 
    ...
    while (status)
    {
        ...
        if(sync_packages(Measures)) 
        {
            ...
            if(!init_map)
            {
                ...
                {
                    read(point_cloud_read, "../PCD/scans.pcd");
                    ikdtree.Build(point_cloud_read->points);
